...
class SpellChecker(object):
    ...
    # index = 0 is set here so that the function can be called for one line and
    # index defaults to 0
    def check_words(self, sentence, index=0):
        ...
        for word in words_to_check:
            if not self.check_word(word):
                print('Word is misspelt ' + word + ' at line : ' +
                      str(index+1) + ' pos ' + str(caret_position+1))
                failed_words.append({'word': word, 'line': index+1,
                                     'pos': caret_position+1})
            # update the caret position to be the length of the word plus 1 for
            # the split character.
